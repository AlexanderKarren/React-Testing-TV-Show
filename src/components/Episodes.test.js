...
test("Component rendering properly", () => {
    render(<Episodes episodes={episodes} />, { wrapper: MemoryRouter });
    
})

test("Component displays all data from object", () => {
    const { queryAllByText } = render(<Episodes episodes={episodes} />, { wrapper: MemoryRouter });
    expect(queryAllByText(/season 1/i)).toHaveLength(8);
})